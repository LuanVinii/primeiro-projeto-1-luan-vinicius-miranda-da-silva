    ...
    default void notifyConsumers(Message message) {
        consumers().forEach(consumer -> {
            ...
            completableFuture.thenAccept(result -> {
                if (result) {
                    System.out.printf("Consumer %s consumed message %s\n", consumer.name(), message.getId());
                    message.setConsumed(true); // Marca a mensagem como consumida
                    message.addConsumption(consumer); // Adiciona o consumo
                } else {
                    System.out.printf("Consumer %s failed to consume message %s\n", consumer.name(), message.getId());
                }
            }).exceptionally(ex -> {
                System.err.printf("Error consuming message %s by consumer %s: %s\n", message.getId(), consumer.name(), ex.getMessage());
                return null;
            });
        });
    }
